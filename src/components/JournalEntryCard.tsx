...
interface JournalEntryCardProps {
  entry: Entry;
  onUpdate: (id: string, updates: Partial<Pick<Entry, 'summary' | 'transcription' | 'location_tag'>>) => void;
...
export function JournalEntryCard({ entry, onUpdate, onDelete, searchQuery = '' }: JournalEntryCardProps) {
  ...
  return (
    <div
      ref={cardRef}
      className={[
        'bg-card border border-border rounded shadow-[0_2px_8px_rgba(0,0,0,0.4)]',
    ...
    >
      ...
      >
        {/* Timestamp column */}
        <div className="shrink-0 w-16 md:w-20 flex flex-col items-center pt-0.5">
          ...
          <span className="text-[9px] text-muted-foreground/60 font-mono text-center leading-tight hidden md:block">
            {date}
          </span>
        </div>
        ...
        <div className="flex-1 min-w-0">
          {editing ? (
            <Input
              ...
              className="text-sm font-semibold bg-input border-primary/60 text-foreground h-8 px-2"
          ...
          {/* Location tag + edit badge */}
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            {entry.location_tag && !editing && (
              <Badge variant="outline" className="text-[10px] border-border text-muted-foreground gap-1 py-0 h-5">
      ...
      {(expanded || editing) && (
        <div className="px-4 pb-4 border-t border-border/50 pt-3 space-y-3">
          ...
          <div className="space-y-1">
            ...
            {editing ? (
              <Textarea
                ...
                className="text-sm bg-input border-border text-foreground min-h-20 resize-none focus-visible:ring-primary px-3"
                placeholder="Full transcription text..."
              />
            ) : (
              <p className="text-sm text-foreground/80 leading-relaxed text-pretty">
                {entry.transcription
                  ? highlightText(entry.transcription, searchQuery)
                  : <span className="text-muted-foreground italic">No transcription captured</span>
                }
              </p>
            )}
          </div>
          ...
          <div className="space-y-1">
            ...
            {editing ? (
              <Input
                ...
                className="text-sm bg-input border-border text-foreground h-9 px-3 focus-visible:ring-primary"
                placeholder="e.g. Sector A, Stairwell B, North entrance..."
              />
            ) : (
              <p className="text-sm text-foreground/80">
                {entry.location_tag || <span className="text-muted-foreground italic">No location tag</span>}
              </p>
            )}
          </div>
          ...
          {/* AI Summary note */}
          {!editing && (
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground border-t border-border/30 pt-2">
